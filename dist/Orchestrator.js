    ...
    async checkForEarlyTermination(taskState, currentStageIndex) {
        ...
        if (currentStageIndex === 0) {
            ...
            if (intakeDecision && !['PROCEED'].includes(intakeDecision)) {
                logger_1.logger.info('Intake recommends halting', { taskId: taskState.taskId, decision: intakeDecision });
                return true;
            }
        }
        ...
        if (currentStageIndex === 1) {
            ...
            if (detectiveOutput.includes('NOT A BUG') ||
                ...
                logger_1.logger.info('Detective recommends halting', { taskId: taskState.taskId });
                return true;
            }
        }
        ...
        if (currentStageIndex === 2) {
            ...
            let closureCount = 0;
            if (intakeOutput.includes('CLOSE') || intakeOutput.includes('NEEDS_MORE_INFO'))
                closureCount++;
            if (detectiveOutput.includes('NOT A BUG') || detectiveOutput.includes('CLOSE'))
                closureCount++;
            if (archaeologistOutput.includes('NOT A BUG') || archaeologistOutput.includes('PROCESS FAILURE'))
                closureCount++;
            if (closureCount >= 2) {
                logger_1.logger.info('Agent consensus recommends halting', { taskId: taskState.taskId, count: closureCount });
                return true;
            }
        }
        return false;
